...
public class Problem11 extends EulerProblem {
  public static int problemNumber = 11;
  public static String title = "Largest product in a grid";
  ...
  public Problem11(int rows, int cols) {
    this.rows = rows;
    this.cols = cols;
  }

  public Problem11(int rows, int cols, int[] grid) {
    this.grid = grid;
  }

  private int getProduct(int idx, int shift) {
    int product = 1;
    for (int i = 0; i < 4; i++) {
      product *= grid[idx + i*shift];
    }
    return product;
  }

  private void check(int product) {
  ...
  public Object solve() {
    ...
    for (int i = 0; i < grid.length; i++) {
      ...
      // no need to do any calculating if number is zero
      if (grid[i] == 0) {
        continue;
      }
      ...
      // figure out in which directions we can get a product
      boolean n = row > (length - 1) - 1;
      boolean w = col > (length - 1) - 1;
      boolean e = col < cols - (length - 1);
      boolean s = row < rows - (length - 1);
